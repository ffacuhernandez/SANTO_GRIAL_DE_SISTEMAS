...
void ordenar_bloque(Area_datos* datos[], Area_indices* indices[], int inicio, int  n, int indice_bloque) {
	...
	for (int i = inicio; i <= inicio + n - 1; ++i) {
		for (int j = inicio; j <= inicio + n - i - 1; ++j) {
			// Si el elemento actual es mayor que el siguiente, los intercambiamos
			if (datos[j]->clave > datos[j + 1]->clave) {
				int temp = datos[j]->clave;
				datos[j]->clave = datos[j + 1]->clave;
				datos[j + 1]->clave = temp;
				
				int temp1 = datos[j]->dir;
				datos[j]->dir = datos[j + 1]->dir;
				datos[j + 1]->dir = temp1;
				
				string temp2 = datos[j]->dato;
				datos[j]->dato = datos[j + 1]->dato;
				datos[j + 1]->dato = temp2;
			}
		}
	}
	...
	int j = inicio;
	// Recorremos y movemos los elementos que no sean cero al principio
	for (int i = inicio; i <= inicio + n; ++i) {
		if (datos[i]->clave != 0) {
			datos[j]->clave = datos[i]->clave;
			datos[j]->dir = datos[i]->dir;
			datos[j]->dato = datos[i]->dato;
			j++;
		}
	}
	while (j <= inicio + n) {
