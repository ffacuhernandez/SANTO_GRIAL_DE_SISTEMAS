...
void rightDeUnNodo(NGrafo* lista_n, int id_nodo);
...
void menu_opcion4 (NGrafo* lista_n){
	...
	cout<<"INGRESE EL ID DEL NODO QUE DESEA HALLAR EL RIGHT: ";
	cin>>x;
	
	rightDeUnNodo(lista_n,x);
	
}


void rightDeUnNodo(NGrafo* lista_n, int id_nodo){
	NGrafo* nodo_actual = lista_n;
	
	while(nodo_actual != nullptr && nodo_actual->id_nodo != id_nodo){
		nodo_actual = nodo_actual->link;
	}
	
	if(nodo_actual == nullptr){
		cout<<"No se encontro el nodo "<<id_nodo<<endl; 
		return;
	}
	
	NArco* destinos = nodo_actual->lista_arco;
	cout<<"El conjunto derecho del nodo "<<id_nodo<<" Es: {";
	while(destinos!=nullptr){
		cout<<destinos->destino->id_nodo;
		
		if(destinos->link !=nullptr){
			cout<<", ";
		}
		
		destinos = destinos->link;
	}
	cout<<" } "<<endl;
...
int main(int argc, char *argv[]) {
	...
	do {
		...
		cout << "4- Hallar El Right De Un Nodo.\n";
		cout << endl;
		cout << "    0- Salir\n";
		cout << endl;
		cout << "                        Ingrese opcion: ";
